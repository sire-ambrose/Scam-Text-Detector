...
file1 = open(r"C:\Users\ANONYMOUS\Documents\fraud-app\fraud.txt", "r", encoding="utf8")
file2 = open(r"C:\Users\ANONYMOUS\Downloads\WhatsApp Chat with MR OLA FIXED MATCH PLATFORM .txt", "r", encoding="utf8")
fraud = file1.read()+file2.read()
#fix = file2.read()

file=pd.read_csv(r'C:\Users\ANONYMOUS\Documents\fraud-app\fraud_csv1.txt', encoding='latin', error_bad_lines=False)
...
if st.button('Submit'):
    if text != str():
        ...
        if NB.predict(se)[0]==1 :
            st.write('## FRAUD')
            st.write('Chances of it being fraudulent is \n',round(NB.predict_proba(se)[0][1]*100))
        else :
            st.write('## NOT FRAUD')
            st.write('Chances of it not being fraudulent is \n',round(NB.predict_proba(se)[0][0]*100))
